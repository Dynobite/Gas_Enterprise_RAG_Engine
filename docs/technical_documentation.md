...
* **Объем**: 32 900+ векторизованных фрагментов документов.
...
### 2.6. Адаптивный OCR на базе Vision VLM (OllamaOCR)
* **Модель**: `minicpm-v:latest` (8B VLM) на GPU NVIDIA RTX A6000.
* **Пайплайн**:
  1. Автоматическая детекция сканированных страниц (`len(text) < 30`).
  2. Высокоточный рендеринг страницы через `pdftoppm` (150–200 DPI).
  3. Распознавание верстки, таблиц, штампов и формул в структурированный Markdown.
  4. Проиндексированы все 92 страницы скана `СТО Газпром 2-4.1-212-2008.pdf`.
...
* **PDF**: Встраивание с поддержкой прямого перехода на `#page=N`.
* Исключает скачивание файлов на диск пользователя.

### 2.9. Верификация фактов (LLM-as-a-Judge)
...
### 2.10. UX-Аналитика и телеметрия по Client IP
...
* **Сервер**: `gpu-compute-node` (IP: `localhost`, OS: Ubuntu 24.04 LTS).
* **Аппаратные ресурсы**:
  - GPU: **NVIDIA RTX A6000 (48 GB GDDR6 VRAM)**.
  - CPU/RAM: 32 Cores, **128 GB RAM**, NVMe Storage.
* **Автономная служба 24/7**:
  - Systemd Unit: `/home/admin/.config/systemd/user/gas_rag.service` (`Restart=always`, `loginctl enable-linger`).
  - Standalone Qdrant Daemon: `/home/admin/RAG_SYSTEM/qdrant_daemon/qdrant` (Port `6333`).
